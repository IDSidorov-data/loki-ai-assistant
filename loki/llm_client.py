...
class OllamaLLMClient:
    ...
    # Метод теперь принимает system_prompt как аргумент
    async def stream_response(self, user_prompt: str, system_prompt: str) -> AsyncGenerator[str, None]:
        try:
            async with self.async_client.stream("POST", f"{self.base_url}/api/generate", json={"model": self.model, "system": system_prompt, "prompt": user_prompt, "stream": True}) as response:
